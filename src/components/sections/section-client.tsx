...
export const SectionClient = () => {
  ...
  // Derive filtered/sorted jobs during render instead of useEffect + state
  const displayedJobs = useMemo(() => {
    let jobs = allJobs.slice(0, 6);

    // Apply filter
    if (appliedFilter) {
      jobs = allJobs
        .filter((job) =>
          job.role.toLowerCase().includes(appliedFilter.toLowerCase())
        )
        .slice(0, 6);
    }
    ...
    return jobs;
  }, [allJobs, sortBy, appliedFilter]);

  // Stable callback for JobCard - no API call needed, just navigation
  const handleApply = useCallback(() => {
    // Navigation is handled by the Link component in JobCard
  }, []);
