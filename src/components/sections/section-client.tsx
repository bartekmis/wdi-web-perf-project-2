...
export const SectionClient = () => {
  ...
  const { data: allJobs = [], isLoading } = useQuery({
    queryKey: ["jobs-client", searchQuery],
    queryFn: async () => {
      const start = performance.now();

      const apiUrl = searchQuery
        ? `${process.env.NEXT_PUBLIC_API_BASE_URL}/jobs?${searchQuery}`
  ...
  // Derive sorted/filtered jobs during render (no effects needed)
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayJobs.map((job: Job) => (
            <JobCard
              ...
              onApply={() => {
                // Job details already available - no need to refetch
